...
type AuthService interface {
	CheckUser(email, password string) (dto.UserDto, string, error)
	Register(userVo vo.UserVo) error
	ForgetPassword(userVo vo.UserVo) error
	Captcha(to []string) error
	Send163Mail(mail models.Mail) error
	CheckCaptcha(email, captchaCode string) error
}
